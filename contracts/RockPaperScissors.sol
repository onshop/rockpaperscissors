...
contract RockPaperScissors is Ownable, Pausable {
    ...
    function play(bytes32 gameHash, bytes32 playerMoveHash) public whenNotPaused {
        ...
        emit PlayerMove(msg.sender, gameHash);
    ...
    function revealPlayerMove(bytes32 gameHash, bytes32 secret, uint8 playerMove) external onlyOwner {
        ...
        bytes32 expectedMoveHash = hashPlayerMove(gameHash, msg.sender, secret, playerMove);
    ...
    function withDrawFromGame(bytes32 gameHash) whenNotPaused external whenNotPaused {
        Game storage game = games[gameHash];
        require(game.stake != 0, GAME_NOT_FOUND_MSG);
        require(msg.sender == game.playerOne || msg.sender == game.playerTwo, INVALID_PLAYER_MSG);
        ...
        emit PlayerWithdraws(msg.sender, gameHash, game.stake);

        balances[msg.sender] = balances[msg.sender].add(game.stake);
    ...
    function hashPlayerMove(bytes32 gameHash, address player, bytes32 secret, uint playerMove) public view returns (bytes32) {
        require(secret != bytes32(0), "Secret cannot be empty");
        require(player != NULL_ADDRESS, "Address cannot be zero");

        return keccak256(abi.encodePacked(gameHash, player, secret, playerMove, address(this)));
    }
